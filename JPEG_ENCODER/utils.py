...
def compress(path, flag_alg):
    ...
    if flag_alg ==3: 
        ...
        #Choose Algo
        result+= _encoder.getTime() + '[INFO] Entropy Encoder\n'
        #Huffman
        if flag_alg==0:
            print(len(img_y))
...
def decompress(path,flag_alg):
    ...
    if flag_alg ==3:
        ...
        #LZW 
        _decoder = decoder()
        result_tmp= _decoder.getTime() + '[INFO] Entropy Decoder\n'
        if flag_alg ==1:
        ...
        result_tmp+= _decoder.getTime() + 'Done\n'
        #decode
